...
async function handleInit(config: CVWorkerConfig): Promise<void> {
  try {
    ...
    // Load the WASM fileset (pinned version rather than @latest for reproducibility)
    const wasmFileset = await FilesetResolver.forVisionTasks(
      'https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.22/wasm'
    );

    // Try GPU delegate first, fall back to CPU if creation fails. GPU can
    // silently fail inside a worker on some machines/drivers, which previously
    // left the pipeline non-functional.
    const createPose = (delegate: 'GPU' | 'CPU') =>
      PoseLandmarker.createFromOptions(wasmFileset, {
        baseOptions: { modelAssetPath: config.poseModelPath, delegate },
        runningMode: 'VIDEO',
    ...
    const createHand = (delegate: 'GPU' | 'CPU') =>
      HandLandmarker.createFromOptions(wasmFileset, {
        baseOptions: { modelAssetPath: config.handModelPath, delegate },
        runningMode: 'VIDEO',
        numHands: 2,
...
function handleProcessFrame(frame: ImageBitmap, timestamp: number): void {
  ...
  try {
    ...
    // NOTE: MediaPipe HandLandmarker does NOT provide per-landmark visibility.
    // If a hand is returned at all, its landmarks are present, so we default
    // visibility to 1 (fully present) rather than 0. Defaulting to 0 caused
    // every downstream visibility check to reject valid hands.
