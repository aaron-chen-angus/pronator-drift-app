...
export class CVWorkerManager {
  ...
  async initialize(config: CVWorkerManagerConfig): Promise<void> {
    ...
    // Try the Web Worker first; if it fails to start (e.g. module workers are
    // blocked, as on the file:// protocol), fall back to running the models on
    // the main thread so detection still works.
    try {
      await this.initializeWorker(config);
    } catch (workerError) {
      console.warn('CV worker init failed, falling back to main thread:', workerError);
      // Clean up the failed worker before switching modes.
      if (this.worker) {
  ...
  private async processFrameMainThread(
    ...
    try {
      ...
      // Default to 1 (present) so downstream visibility checks don't reject valid hands.
      const handLandmarks =
