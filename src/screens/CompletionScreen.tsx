...
interface CompletionScreenProps {
  ...
  /**
   * Optional builder called at the moment results are shown, so the real
   * analysis result (finalized right at the end of the timed assessment) is
   * used rather than a value captured at render time. Preferred over
   * `assessment` when provided.
   */
  buildAssessment?: () => PronatorDriftAssessment;
}
 ...
 */
export function CompletionScreen({ dispatch, speechSystem, assessment, buildAssessment }: CompletionScreenProps) {
  ...
  const showResults = useCallback(() => {
    ...
    // Prefer the fresh builder (real analysis result finalized at ~30s) over a
    // value captured at render time.
    const finalAssessment = buildAssessment ? buildAssessment() : assessment;
    if (finalAssessment) {
      dispatch({ type: 'SHOW_RESULTS', assessment: finalAssessment });
    }
  }, [dispatch, assessment, buildAssessment]);
